...
tasksTimerCreatePreset.create_icon = function(timers) {
...
TasksTimerMenuItem.addTimerStart = function(result, timers) {
  if (timers === undefined) {
...
TasksTimerMenuItem.re_alarm = function(parse) {
...
TasksTimerMenuItem.re_hms = function(parse) {
  var re = /^((\d+):)?((\d+):)?(\d+)$/;
...
TasksTimerMenuItem.re_name_hms = function(parse) {
  var re = /^(.*?)\s+(\d+):(\d+):(\d+)$/;
  var m = re.exec(parse.entry);
  if (m) {
...
tasksTimerMenuItem.re_wildcard = function(parse) {
...
tasksTimerMenuItem.parseTimerEntry = function(entry, quick) {
  if (entry.length === 0) {
    return undefined;
  }
