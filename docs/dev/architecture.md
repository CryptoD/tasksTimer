...
| `rateLimitMiddleware` on `POST /login` | **N/A** — no HTTP login API. **No account lockout** by policy — [ADR 0002](adr/0002-account-lockout-rate-limit-only.md); reference [`tooling/auth_abuse_policy.mjs`](../../tooling/auth_abuse_policy.mjs). |
